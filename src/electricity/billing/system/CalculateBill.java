...
//        lblcity.setBounds(100, 120, 100, 20);
//        p.add(lblcity);
//        
//        tfunits = new JTextField();
//        tfunits.setBounds(240, 120, 200, 20);
//        p.add(tfunits);
//        
...
//        
//        cmonth = new Choice();
//        cmonth.setBounds(240, 160, 200, 20);
//        cmonth.add("January");
//        cmonth.add("February");
//        cmonth.add("March");
//        cmonth.add("April");
...
//    try {
//        Conn c = new Conn();
//        ResultSet rs = c.s.executeQuery("SELECT * FROM login WHERE meter_no = '" + meternumber.getSelectedItem() + "'");
//        if (rs.next()) {
//            lblname.setText(rs.getString("name"));
//            labeladdress.setText(rs.getString("address"));
//        } else {
...
//}
//    
//    // Populate meter number dropdown
...
public class CalculateBill extends JFrame implements ActionListener {
    ...
    CalculateBill() {
        ...
        JLabel lblnameTitle = new JLabel("Customer Name");
        lblnameTitle.setBounds(100, 120, 100, 20);
        p.add(lblnameTitle);
        ...
        lbladdressTitle.setBounds(100, 160, 100, 20);
        p.add(lbladdressTitle);
        ...
        // Add listener for meter number changes
        meternumber.addItemListener(e -> updateCustomerDetailsAsync());
        
        setVisible(true);
    }

    private void updateCustomerDetailsAsync() {
        new SwingWorker<Void, Void>() {
            @Override
            protected Void doInBackground() throws Exception {
                try {
                    Conn c = new Conn();
                    String query = "SELECT c.name, c.address FROM customer c " +
                                 "JOIN login l ON c.meter_no = l.meter_no " +
                                 "WHERE c.meter_no = '" + meternumber.getSelectedItem() + "'";
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == next) {
            ...
            if (units.isEmpty() || !units.matches("\\d+")) {
            ...
            new SwingWorker<Void, Void>() {
                ...
                protected Void doInBackground() throws Exception {
                    try {
                        ...
                        Conn c = new Conn();
                        Future<ResultSet> taxFuture = c.executeQueryAsync("SELECT * FROM tax");
                        ResultSet taxRs = taxFuture.get();

                        while (taxRs.next()) {
                            totalbill += unit_consumed * Integer.parseInt(taxRs.getString("cost_per_unit"));
                        ...
                        String query = "INSERT INTO bill VALUES('" + meter + "', '" + month + 
                                     "', '" + units + "', '" + totalbill + "', 'Not Paid')";
                        Future<Integer> updateFuture = c.executeUpdateAsync(query);
                        updateFuture.get();
